...
def upload():
    ...
    if response.status_code != 200:
        # Fallback to OCR if annotation not found
        text = pytesseract.image_to_string(Image.fromarray(img), lang='pan')
        return render_template("index.html", text=text if text else "")
